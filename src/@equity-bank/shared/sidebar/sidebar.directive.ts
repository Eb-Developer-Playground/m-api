...
@Directive({
  selector: '[equityBankSidebar],equity-bank-sidebar',
  host: {
    class: 'equity-bank-sidebar',
  },
  exportAs: 'equityBankSidebar',
})
export class SidebarDirective implements AfterViewInit, OnDestroy {
  ...
  enableScrollblock() {
    this.document.body.classList.add('equity-bank-scrollblock');
  }

  disableScrollblock() {
    this.document.body.classList.remove('equity-bank-scrollblock');
  }
